$(function () {
    ...
    function switchSkuId() {
        ...
        console.log("len:" + redborderDivs.length)
        var saleValueIds = "";
        $.each(redborderDivs,function (i, n) {

            if (i>0){
                saleValueIds +="|";
            }
            //console.log($(n).attr('value'));
            saleValueIds += $(n).attr('value');
        })
        console.log("saleValueIds:" + saleValueIds);
        //获取该spu所有的sku的销售属性值id组
        var saleValueSkuIdJSON = $("#saleValueSkuIdJSON").val();
        console.log("saleValueSkuIdJSON:" + saleValueSkuIdJSON);
        var saleValueSkuId = JSON.parse(saleValueSkuIdJSON);
        //将页面选中的销售属性值id组与数据库中该spu所有的sku的销售属性值id组匹配，并得到目标skuid
        var TargetSkuId = saleValueSkuId[saleValueIds];
        console.log("TargetSkuId:" + TargetSkuId);
        var skuIdSelf = $("#skuId").val();
        //判断目标skuid，并跳转
        if (TargetSkuId) {
            if (TargetSkuId == skuIdSelf) {
                $("#cartBtn").attr("class","box-btns-two");
                $("#cartBtn").attr("canClick",'1');
                $("#cartBtn").css("cursor",'pointer') ;
            } else {
                window.location.href = "/" + TargetSkuId + ".html";
            }
        } else {
